...
router.route("/addlab").post(upload.single("labsImages"), async (req, res) => {
  const { name, address, id, lat, lng, description } = req.body;
  if (id) {
    let imgPath;
    let lab;
    try {
      ...
      if (lat !== 36.24586065214592 && newLab.location.lat !== lat) {
        newLab.location = { lat: lat, lng: lng };
      }
      newLab.description = description;
