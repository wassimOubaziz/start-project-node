...
router.get("/getanalyse", async (req, res) => {
  ...
  try {
    ...
    if (lab) {
      ...
      return res.json({ analyses, hasJob: req.user.hasJob });
    } else {
      return res.status(400).json({
        message: "Worker ID not found in any lab.",
        hasJob: req.user.hasJob,
      });
    }
  } catch (err) {
    console.log(err.message);
    res.status(400).json({ message: err.message, hasJob: req.user.hasJob });
  }
});

//  know the id of laboratory of reseptioniste

router.get("/getlab_id", async (req, res) => {
